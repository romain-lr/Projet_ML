...
boost_model <- gbm(CO2 ~ ., data = train_data, distribution = "gaussian", n.trees = 500, 
...
#gbm_rmse <- RMSE(gbm_pred, test_data$CO2)
...
#cat("RMSE pour le modèle GBM :", gbm_rmse, "\n")
```
...
ggplot(predictions, aes(x = True)) +
  ...
  labs(title = "Comparaison des prédictions par rapport aux vraies valeurs",
       x = "Vraie valeur de CO2",
       y = "Valeur prédite",
       color = "Modèles") + 
  scale_color_manual(values = c("blue", "red", "green"), labels = c("LM_AIC", "SVM", "Tree")) + # 
  theme_minimal()
