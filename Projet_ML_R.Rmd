...
gplot.res <- function(x, y, titre = "titre"){
    ...
    geom_point(col = "blue")+xlim(0, 2500)+ylim(-500000, 500000)+
...
set.seed(2021)
#svm.reg.tune = tune.svm(CO2 ~ ., data = train_data, cost = c(1,2,3,4,5), 
...
#Il faut d'abord faire un encodage one-hot de la variable Entity car il y a trop de niveaux (148) : une classe sera maintenant représentée par une variable binaire

entity_encoded <- model.matrix(~ Entity - 1, data = train_data)

#On remplace aussi les caractères spéciaux par des "_" pour le nouveau df

colnames(entity_encoded) <- gsub("[^[:alnum:]]", "_", colnames(entity_encoded))

#On forme le nouveau échantillon d'apprentissage en concaténant les matrices encodées avec les autres variables

train_data_encoded <- cbind(train_data[, -which(names(train_data) == "Entity")], entity_encoded)
...
#On créé le modèle randomForest

rf_model <- randomForest(CO2 ~ ., data = train_data_encoded, ntree = 500)
...
```{r}
#Avant la question 3 on doit également encoder les données de test (comme celles d'entrainement) pour faire la prédiction avec Random Forest

#On applique l'encodage one-hot sur les données de test
entity_encoded_test <- model.matrix(~ Entity - 1, data = test_data)

#On remplace les caractères spéciaux par des "_"
colnames(entity_encoded_test) <- gsub("[^[:alnum:]]", "_", colnames(entity_encoded_test))


test_data_encoded <- cbind(test_data[, -which(names(test_data) == "Entity")], entity_encoded_test)
